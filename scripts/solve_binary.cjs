...
const groups = [
  ...
  { indices: [323], choices: [0, 1] },
  { indices: [357], choices: [0, 1] },
  { indices: [358], choices: [0, 1] },
  { indices: [430], choices: [0, 1] },
  { indices: [431], choices: [0, 1] }
];
...
console.log(`Split template into ${staticParts.length} static parts.`);
...
console.log(`Starting binary search over ${totalCombinations} combinations...`);
...
search(0);
console.log(`Binary search finished in ${Date.now() - startTime}ms`);
if (!found) {
  console.log("Binary search failed to find match.");
}
