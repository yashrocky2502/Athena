...
modIdxs.forEach(i => {
  if (i >= 433 && i <= 444) {
    if (i === 433 || i === 434) {
      // Record 433 and 434: original lastUpdated is "2026-09-17T08:42:04.137Z"
      const gen = new Date(recs[i].generatedAt).getTime();
      const updated = new Date("2026-09-17T08:42:04.137Z").getTime();
      knownDiffs[i] = updated - gen;
    } else {
      knownDiffs[i] = 1;
    }
  } else if (i === 445) {
    // Record 445: original lastUpdated is "2026-08-30T12:48:01.563Z" (equal to generatedAt)
    knownDiffs[i] = 0;
  } else if (i >= 152 && i <= 204) {
    knownDiffs[i] = 0;
  } else if (i >= 226 && i <= 228) {
    knownDiffs[i] = 0;
  } else if ([211, 219, 222, 231, 256, 335, 336, 362, 384, 385].includes(i)) {
    knownDiffs[i] = 1;
  }
});
...
const groups = [
  ...
  { indices: [430], choices: [1, 7, 62] },
  { indices: [431], choices: [1, 7, 62] }
];
...
// If no argument, run as MASTER
if (process.argv.length < 3) {
  // Calculate total combinations
  let totalCombs = 1;
  groups.forEach(g => { totalCombs *= g.choices.length; });
  console.log(`MASTER: Spawning 4 worker processes to search ${totalCombs.toLocaleString()} combinations in parallel...`);
  ...
  for (let i = 0; i < 4; i++) {
    const worker = fork(__filename, [String(i)]);
    workers.push(worker);

    worker.on("message", (msg) => {
      if (msg && msg.type === "SUCCESS") {
        success = true;
        console.log(`MASTER: Worker ${i} found the MATCH! Terminating other workers...`);
        ...
        console.log("MASTER: Done! Successfully saved restored datasets to disk.");
        process.exit(0);
      }
    });
    ...
    worker.on("exit", () => {
      finishedCount++;
      if (finishedCount === 4) {
        console.log(`MASTER: All workers finished in ${Date.now() - startTime}ms.`);
        if (!success) {
          console.log("MASTER: FAIL - No match found by any worker.");
          process.exit(1);
        }
      }
    });
  ...
  const choice0 = workerId >> 1; // choice index for Group 0
  const choice1 = workerId & 1;  // choice index for Group 1
  
  console.log(`WORKER ${workerId}: Starting partition (G0=${choice0}, G1=${choice1})...`);

  // Apply worker's partition choices
  const setGroupChoice = (gIdx, cIdx) => {
  ...
  setGroupChoice(0, choice0);
  setGroupChoice(1, choice1);
  ...
  // Start search from Group 2 (since Group 0 and 1 are fixed for this worker)
  search(2);
  console.log(`WORKER ${workerId}: Partition search complete.`);
}
