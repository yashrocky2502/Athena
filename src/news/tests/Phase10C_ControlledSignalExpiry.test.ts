 ...
 *
 * Test Matrix (Requirements A through W):
 ...
 * - TEST T: VITEST environment prevents unintended background timer execution
...
import { SignalLifecycleEngine, SignalLifecycle } from '../intelligence/SignalLifecycleEngine.ts';
...
describe('PHASE 10C — CONTROLLED SIGNAL EXPIRY ENGINE', () => {
  ...
  it('TEST A: valid live ACTIVE signal expires after its validity window', async () => {
    ...
    // 1. Seed in isolated Outcome Engine
    ...
    // 2. Register in live expiry source
    ...
    // 4. Verify outcome engine record synchronization
  ...
  it('TEST G: CONTRADICTED signal follows existing deterministic terminal rules and does not bypass lifecycle validation', async () => {
    ...
    isolatedOutcomeEngine.registerActionableSignal({
      ...
      initialPrice: 1800,
      direction: 'BULLISH',
      signalLifecycleState: 'CONTRADICTED'
    });
  ...
  it('TEST K: same symbol existing in both historical and live sources only causes the LIVE signal to be processed', async () => {
    ...
    // 2. Register live TCS signal
    isolatedOutcomeEngine.registerActionableSignal({
      signalId: liveSignalId,
      eventId: 'evt_live_tcs_p10c',
      signalType: 'BREAKING',
      symbol: 'TCS',
      revision: 1,
  ...
  it('TEST L: canonical Phase 10A signalId is preserved', async () => {
    ...
    isolatedOutcomeEngine.registerActionableSignal({
      ...
      eventId: 'evt_canonical_p10a_555',
  ...
  it('TEST Q: no final price or P&L is fabricated upon expiry', async () => {
    ...
    const record = isolatedOutcomeEngine.getRecord(signalId);
    expect(record?.lastObservedPrice).toBeUndefined();
  ...
  it('TEST T: VITEST environment prevents unintended background timer execution', () => {
    // In VITEST environment, start() must NOT start background timer unless autoStartInTest: true
    expiryEngine.start();
    expect(expiryEngine.isActive()).toBe(false);

    // When autoStartInTest is explicitly enabled for tests, it does start
