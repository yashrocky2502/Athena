  ...
  public async getEquityObservation(symbol: string): Promise<EquityObservation | null> {
    ...
    try {
      ...
      const norm = MarketDataNormalizer.normalizeEquity(raw, {
        provider: raw.provenance?.provider || this.name,
        providerType: raw.provenance?.providerType || 'FALLBACK_PROVIDER',
        exchange: raw.provenance?.exchange || 'NSE',
        sourceConfidence: raw.provenance?.sourceConfidence ?? 0.8
      });
