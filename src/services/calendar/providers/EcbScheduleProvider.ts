...
export class EcbScheduleProvider implements IScheduleProvider {
  public readonly providerName = 'ECB Governing Council Schedule Provider';
  ...
  public async fetchEvents(): Promise<ScheduledEvent[]> {
    ...
    try {
      ...
      const apiResp = await fetch('/api/calendar/ecb', {
      ...
      const response = await fetch('https://www.ecb.europa.eu/rss/press.html', {
      ...
      if (response && response.ok) {
        ...
        for (const match of matches) {
          ...
          const url = match[2]?.trim() || 'https://www.ecb.europa.eu';
          const pubDate = match[3]?.trim() || '';

          if (title.toUpperCase().includes('MONETARY POLICY') || title.toUpperCase().includes('GOVERNING COUNCIL') || title.toUpperCase().includes('RATES')) {
            const releaseTimeUTC = pubDate ? new Date(pubDate).toISOString() : new Date().toISOString();
            events.push({
              id: `ecb_${idx}_${Buffer.from(title).toString('hex').slice(0, 10)}`,
              title,
              country: 'Euro Area',
              countryCode: 'EU',
              region: 'ECB',
              ...
              announcementDocRef: `ECB-GOV-${releaseTimeUTC.slice(0, 10)}`
            });
          }
        }
      }
